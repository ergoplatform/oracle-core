...
There is a *outlier range* which all oracles must be within in order to get rewarded for being accurate in the given epoch. They must be plus or minus the outlier range compared to the previous finalized datapoint. If they are not accurate, the current scheme does not pay them out (rather than slashing which is a bit aggressive).
...
13. The blake2b256 hash of Output #1's address is equal to the hash held in R6 of Input #1.
14. Every data-input [Datapoint](<#Stage-Datapoint>) box has a datapoint within the outlier range.
---
