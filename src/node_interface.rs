...
use ergo_utilities::node_interface::{NodeError, NodeInterface};
use ergo_utilities::{BlockHeight, P2PKAddress, P2SAddress, ScanID, TxId};
use json::JsonValue;
...
pub fn send_transaction(tx_request_json: &JsonValue) -> Result<TxId> {
    new_node_interface().send_transaction(tx_request_json)
}
...
pub fn address_to_tree(address: &P2SAddress) -> Result<String> {
    new_node_interface().address_to_tree(address)
}
...
pub fn address_to_bytes(address: &P2SAddress) -> Result<String> {
    new_node_interface().address_to_bytes(address)
}
...
pub fn address_to_raw(address: &P2PKAddress) -> Result<String> {
    new_node_interface().address_to_raw(address)
}
...
pub fn raw_from_register_to_address(typed_raw: &String) -> Result<P2PKAddress> {
    new_node_interface().raw_from_register_to_address(typed_raw)
}
